...
export default function getItemText(item: Pokemon) {
  // PokemonNameData에서 일치하는 id가 일치하는 아이템을 찾음
  const findItem = PokemonNameData.find((mock) => mock.id === item.id);
  const koreanName = findItem?.ko_name ?? "";
  const englishName = findItem?.en_name ?? "";
  const japaneseName = findItem?.jp_name ?? "";
